...
internal static class LibLoader
{
    ...
    private static IntPtr Resolve(string name, Assembly assembly, DllImportSearchPath? searchPath)
    {
        ...
        {
            if (name == "libxdg-shell-client")
            {
                var (rid, fileName) = ("linux-x64", "libxdg-shell-client.so");

                string resourceName = $"Angene.Linux.Native.{rid}.{fileName}";
